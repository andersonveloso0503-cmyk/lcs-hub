...
// Tenta pegar uma foto real do Banco de Temas (theme_bank) pro serviço do
// dia. Retorna null se não tiver nenhuma cadastrada pra esse serviço —
// nesse caso o chamador cai de volta pra geração via IA.
async function pickBankPhoto(db, aiKey) {
  try {
    const q = query(collection(db, "theme_bank"), where("service", "==", aiKey));
    const snap = await getDocs(q);
    if (snap.empty) return null;
    const docs = snap.docs.map((d) => d.data());
    const chosen = docs[Math.floor(Math.random() * docs.length)];
    return chosen.imageUrl || null;
  } catch (err) {
    console.error(`[auto-week] Erro ao buscar foto do banco (${aiKey}):`, err.message);
    return null;
  }
}
  ...
  try {
    ...
    for (let i = 0; i < posts.length; i++) {
      ...
      try {
        ...
        const aiKey = SERVICE_TO_AI_KEY[post.service] || "Limpeza";

        // Prioriza foto real do Banco de Temas; só gera por IA se não tiver
        // nenhuma cadastrada pra esse serviço.
        let imageUrl = await pickBankPhoto(db, aiKey);
        let imageSource = "banco";

        if (!imageUrl) {
          console.log(`[auto-week] Sem foto no banco para "${aiKey}", gerando via IA (${provider})...`);
          ...
          imageUrl = await uploadToBlob(base64, filename);
          imageSource = "ia";
        } else {
          console.log(`[auto-week] ${post.day}: usando foto real do banco (${aiKey}).`);
        }
        ...
        await addDoc(collection(db, "posts"), {
          ...
          bufferPostIds,
          imageSource,
          aiProvider: imageSource === "ia" ? provider : null,
          autoGenerated: true,
          autoPublished: true,
