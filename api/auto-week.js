...
import { createPostForChannel } from "./buffer-schedule.js";
import {
  generateReelScript,
...
// ── Reels automáticos (3x/semana: Segunda, Quarta, Sexta) ──────────────────
// Cada dia o cron: 1) tenta AVANÇAR qualquer reel que ficou pendente de um
// dia anterior (render do Shotstack ou processamento do Instagram que não
// terminou a tempo), e 2) se hoje for dia de reel novo, inicia um.
// Tudo com um orçamento de tempo (maxDuration 60s na Vercel) — se não der
// tempo de terminar hoje, salva o progresso e o cron de amanhã continua.
...
function pickReelTheme() {
  return REEL_THEMES[Math.floor(Math.random() * REEL_THEMES.length)];
}
...
// Tenta avançar UM reel que já está em andamento (rendering ou aguardando
// publicação no Instagram). Faz polling só até o orçamento de tempo acabar.
async function advancePendingReel(db, reelDoc, deadline) {
  const ref = doc(db, "reels", reelDoc.id);
  const data = reelDoc.data();

  if (data.status === "rendering") {
    while (timeLeftMs(deadline) > 8000) {
      const { status, url } = await checkShotstackRenderStatus(data.renderId);
      if (status === "done") {
        const videoUrl = await copyVideoToBlob(url);
        const igAccountId = await getInstagramAccountId();
        const containerId = await createReelContainer(igAccountId, videoUrl, data.caption);
        await updateDoc(ref, {
  ...
  if (data.status === "container_created") {
    while (timeLeftMs(deadline) > 5000) {
      const result = await checkAndPublishReel(data.igAccountId, data.containerId);
      if (result.published) {
        await updateDoc(ref, {
          status: "publicado",
  ...
  return { ok: true, published: false, stillPending: true };
}
...
async function startNewReel(db, deadline) {
  ...
  return advancePendingReel(db, { id: docRef.id, data: () => ({ renderId, caption: script.caption, status: "rendering" }) }, deadline);
}

async function runReelFlow(db, deadline) {
  const results = [];

  // 1) Avança reels pendentes de dias anteriores
  const pendingQ = query(
    collection(db, "reels"),
    where("status", "in", ["rendering", "container_created"])
  );
  const pendingSnap = await getDocs(pendingQ);
  for (const reelDoc of pendingSnap.docs) {
    if (timeLeftMs(deadline) < 8000) break;
    try {
      const r = await advancePendingReel(db, reelDoc, deadline);
      results.push({ id: reelDoc.id, kind: "avançado", ...r });
    } catch (err) {
      console.error(`[auto-week] Erro ao avançar reel ${reelDoc.id}:`, err.message);
      results.push({ id: reelDoc.id, kind: "avançado", ok: false, error: err.message });
    }
  }

  // 2) Inicia um reel novo, se hoje for dia de reel e ainda tiver orçamento
  const todayIndex = new Date().getDay();
  if (REEL_DAYS.includes(todayIndex) && timeLeftMs(deadline) > 15000) {
    try {
      const r = await startNewReel(db, deadline);
      results.push({ kind: "novo", ok: true, ...r });
    } catch (err) {
      console.error("[auto-week] Erro ao iniciar novo reel:", err.message);
      results.push({ kind: "novo", ok: false, error: err.message });
    }
  }
...
export default async function handler(req, res) {
  // Orçamento de tempo pra essa execução — a Vercel mata a function depois
  // de maxDuration (60s, configurado no vercel.json). Deixa uma margem de
  // segurança de 5s pra responder antes disso.
  const functionDeadline = Date.now() + 55000;
  ...
  try {
    ...
    // 3.5) Reels automáticos — roda TODO dia (independente do dia de geração
    // semanal do Instagram), pra avançar reels pendentes e, nas 3
    // segundas/quartas/sextas, iniciar um novo.
    try {
      const reelResults = await runReelFlow(db, functionDeadline);
      response.reels = { results: reelResults };
    } catch (reelErr) {
      console.error("[auto-week/reels] Erro fatal:", reelErr);
      response.reels = { error: reelErr.message };
    }
    ...
    for (let i = 0; i < posts.length; i++) {
      ...
      try {
        ...
        const imageSource = "ia";

        // Agenda direto no Buffer (Instagram) — sem esperar aprovação manual.
        // O registro no Firestore fica como histórico/log, com o status
        // refletindo se o agendamento no Buffer deu certo.
        const scheduleResult = await createPostForChannel("instagram", {
          text: post.caption,
          imageUrls: [imageUrl],
          scheduledAt,
        });

        const status = scheduleResult.ok ? "agendado" : "erro_agendamento";
        const bufferPostIds =
          scheduleResult.ok && scheduleResult.post?.id ? [scheduleResult.post.id] : [];
        ...
        await addDoc(collection(db, "posts"), {
          ...
          autoGenerated: true,
          autoPublished: true,
          ...(scheduleResult.ok ? {} : { scheduleError: scheduleResult.error }),
          createdAt: serverTimestamp(),
        });
        ...
        igResults.push({
          ...
          error: scheduleResult.ok ? undefined : scheduleResult.error,
        });
        console.log(
          scheduleResult.ok
            ? `[auto-week] ✅ ${post.day} agendado no Buffer.`
            : `[auto-week] ⚠️ ${post.day} salvo, mas falhou ao agendar: ${scheduleResult.error}`
        );
      } catch (err) {
    ...
    console.log(`[auto-week] Concluído — ${ok}/${posts.length} posts agendados no Buffer.`);
